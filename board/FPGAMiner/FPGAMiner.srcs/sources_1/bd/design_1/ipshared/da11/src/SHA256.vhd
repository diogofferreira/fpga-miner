...
entity SHA256 is
	...
	port(
        ...
        digest          : out std_logic_vector(HASH_LENGTH-1 downto 0)
    );
end SHA256;
...
architecture Behavioral of SHA256 is
    -- Sha256 signals
    signal W                      : wordArray;
    signal M                      : BlockM ((kCalculator(INPUT_MESSAGE_LENGTH) + INPUT_MESSAGE_LENGTH + 1 + 64 )/ 512 - 1 downto 0 ) := ( (others => ( others => '0' ) ) );
    ...
    signal hashes  : std_logic_vector(HASH_LENGTH-1 downto 0) := (others => '0');
begin
    ...
    process(clk, reset, enable, hashes)
    ...
    begin
        if (reset = '1') then
            ...
            hashIt := 0;
            N := 0;
            initializeH(hashes, constHashes);
        elsif (rising_edge(clk) and enable = '1') then
            -- If diggest is not estimated yet and input message is not padded.
            if (ready = '0' and padded = '0') then
                N := (kCalculator(INPUT_MESSAGE_LENGTH) + INPUT_MESSAGE_LENGTH + 1 + 64)/512; -- Count total (512-bit) blocks in the padded message.
                M <= messagePadding(input_word, INPUT_MESSAGE_LENGTH); -- Get padded message to M(N) blocks.
