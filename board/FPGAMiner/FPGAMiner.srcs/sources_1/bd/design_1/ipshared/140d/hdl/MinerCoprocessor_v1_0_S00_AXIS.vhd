...
architecture Behavioral of MinerCoprocessor_v1_0_S00_AXIS is
    ...
    signal s_hashInputWord        : std_logic_vector(BLOCKCHAIN_BLOCK_HEADER_LENGTH + NONCE_LENGTH - 1 downto 0);
...
begin    
            ...
            -- Input word builder 
                ...
                if (s_enable = '0') then
                    if (s_counter = 20) then
                        ...
                        s_hashInputWord <= s_hashOriginalInputWord & std_logic_vector(to_unsigned(s_nonce, NONCE_LENGTH));
    ...
    sha25_pipeline: entity work.SHA256(Behavioral)
        generic map(
            INPUT_MESSAGE_LENGTH  => BLOCKCHAIN_BLOCK_HEADER_LENGTH + NONCE_LENGTH
        )
        port map(
            clk             => S_AXIS_ACLK,
            reset           => s_reset,
            enable          => s_enable,
            input_word      => s_hashInputWord,
            done            => s_ready,
            digest          => s_hash
        );
