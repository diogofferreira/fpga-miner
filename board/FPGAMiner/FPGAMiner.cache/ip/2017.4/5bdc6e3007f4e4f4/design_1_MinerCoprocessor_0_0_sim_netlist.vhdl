...
architecture STRUCTURE of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_MinerCoprocessor_v1_0_S00_AXIS is
  ...
  signal \^counterslave\ : STD_LOGIC_VECTOR ( 2 downto 0 );
...
begin
  DI(0) <= \^di\(0);
  E(0) <= \^e\(0);
  counterSlave(2 downto 0) <= \^counterslave\(2 downto 0);
  validData <= \^validdata\;
...
\s_counter[3]_i_1__0\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"0004"
    )
        port map (
...
s_validOut_i_4: unisim.vcomponents.LUT4
    generic map(
      INIT => X"FFFE"
    )
        port map (
...
s_validOut_i_7: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFFFFFE"
    )
        port map (
