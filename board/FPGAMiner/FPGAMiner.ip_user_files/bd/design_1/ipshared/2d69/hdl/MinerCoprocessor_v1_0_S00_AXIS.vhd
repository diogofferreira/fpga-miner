...
entity MinerCoprocessor_v1_0_S00_AXIS is
	generic (
        ...
        HASHED_MESSAGE_LENGTH   : integer   := 512;
...
begin
    ...
    begin
        if (rising_edge(S_AXIS_ACLK)) then
            ...
            if (readEnable = '1' and s_validOut = '1') then
                if (allWordsSent = '1') then
                    s_validOut <= '0';
                    s_hashOriginalInputWord <= (others => '0');
                end if;
                s_hashOriginalInputWord <= (others => '1');
                s_dataOut <= s_hashOriginalInputWord;
                --s_enable <= '1';
                --s_counter <= 0;
              -- Miner management
--            elsif (s_enable = '1' and ready = '1') then
...
--                else
--                    s_nonce <= s_nonce + 1;
--                    s_hashInputWord <= s_hashOriginalInputWord & std_logic_vector(to_unsigned(s_nonce, 256));
--                    s_POFready <= '0';
--                    s_update <= '1';
--                end if;
            end if;
    ...
    hasher: process(S_AXIS_ACLK, S_AXIS_ARESETN, s_enable)
